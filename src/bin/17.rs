...
fn reset_if_needed(mut grid: Grid2D<char>) -> Grid2D<char> {
    let min = grid
        .iter()
        .filter(|x| *x.1 != ' ')
        .map(|x| x.0)
    ...
    for row in min..=max {
        if (0..7).all(|i| grid[Coordinate2D(i, row)] != ' ') {
            grid.retain(|c, _| c.1 <= row + 10);
            grid[Coordinate2D(0, max)] = '#';
            return grid;
        }
    }
...
fn main() {
    ...
    let rocks = ROCKS
        .split("\n\n")
        .map(|r| parse_grid(r, |f| if f == '#' { '#' } else { ' ' }, ' '))
        .collect_vec();

    let mut rock_n = 0;

    let mut grid: DefaultHashMap<Coordinate2D, char> = DefaultHashMap::new(' ');
    for i in 0..7 {
        grid[Coordinate2D(i, 0)] = '#';
    }
    ...
    let start_count = (cycles * 4).max(2022);
    ...
    for i in 0..start_count {
        ...
        while let Some(&c) = input.peek() {
            ...
            if check_overlap(&rock, grid.clone(), corner) {
                corner = corner.up(1);
                break;
            }
            ...
            match c {
                '<' => {
                    if left > 0 && !check_overlap(&rock, grid.clone(), corner.left(1)) {
                        corner = corner.left(1)
                    }
                }
                '>' => {
                    if right < 7 && !check_overlap(&rock, grid.clone(), corner.right(1)) {
...
fn check_overlap(
    rock: &DefaultHashMap<Coordinate2D, char>,
    grid2: DefaultHashMap<Coordinate2D, char>,
    corner: Coordinate2D,
) -> bool {
