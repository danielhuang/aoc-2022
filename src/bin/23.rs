...
fn main() {
    ...
    let mut grid = parse_grid(&input, |c| c, '.');
    ...
    for i in 1.. {
        ...
        for (&c, &elf) in grid.iter() {
            if elf == '#' && c.adjacent_corners().iter().any(|x| grid[x] == '#') {
        ...
        if i == 10 {
            let b = bounds(grid.iter().filter(|x| *x.1 == '#').map(|x| *x.0));
            cp(b.area() - grid.iter().filter(|x| *x.1 == '#').count().int());
        }
