...
fn main() {
    ...
    for part2 in [false, true] {
        ...
        for _ in 0..(if part2 { 10 } else { 1 }) {
            for fullnum in orig.clone() {
                let pos = input.iter().position(|&x| x == fullnum).unwrap();
                input.remove(pos);
                let new_pos = (pos.int() + fullnum.1).rem_euclid(input.len().int());
                input.insert(new_pos.uint(), fullnum);
            }
        }

        let zero_pos = input.iter().position(|&x| x.1 == 0).unwrap();
        ...
        cp(expanded[1000] + expanded[2000] + expanded[3000]);
    }
}
