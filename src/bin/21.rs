...
fn find_value(name: &str, map: &HashMap<String, String>) -> i64 {
    if let Ok(x) = map[name].parse() {
        x
    } else {
        let (a, op, b) = map[name].split_whitespace().collect_tuple().unwrap();
        let a = find_value(a, map);
        let b = find_value(b, map);
        match op {
            "+" => a + b,
            "-" => a - b,
            "*" => a * b,
...
enum Expr {
    Value(i64),
    Calc(Box<Expr>, Op, Box<Expr>),
    Unknown,
}

fn find_expr(name: &str, map: &HashMap<String, String>) -> Expr {
    if name == "humn" {
        return Expr::Unknown;
    }
    if let Ok(x) = map[name].parse::<i64>() {
        Expr::Value(x)
    } else {
        let (a, op, b) = map[name].split_whitespace().collect_tuple().unwrap();
        let a = find_expr(a, map);
        let b = find_expr(b, map);
        let op = match op {
            "+" => Op::Add,
            "-" => Op::Sub,
            "*" => Op::Mul,
        ...
        Expr::Calc(Box::new(a), op, Box::new(b))
    }
}

fn eval(expr: &Expr, unknown_value: i64) -> Rational64 {
    match expr {
        Expr::Value(x) => (*x).into(),
        Expr::Calc(a, op, b) => {
            let a = eval(a, unknown_value);
            let b = eval(b, unknown_value);
            match op {
                Op::Add => a + b,
                Op::Sub => a - b,
                Op::Mul => a * b,
        ...
        Expr::Unknown => unknown_value.into(),
    }
}

fn solve_linear(left: &Expr, right: &Expr) -> i64 {
    let at_0 = eval(right, 0) - eval(left, 0);
    let at_1 = eval(right, 1) - eval(left, 1);
    let slope = at_1 - at_0;
    -(at_0 / slope).int()
}

fn main() {
    let input = load_input(21);

    let mut map = HashMap::new();

    for line in input.lines() {
        let (name, op) = line.split_once(": ").unwrap();
        map.insert(name.to_string(), op.to_string());
    }

    cp(find_value("root", &map));

    let (left, _, right) = map["root"].split_whitespace().collect_tuple().unwrap();

    let left = find_expr(left, &map);
    let right = find_expr(right, &map);

    cp(solve_linear(&left, &right));
}
